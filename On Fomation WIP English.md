...
<a href="#Density-Limit-1_u" id="Density-Limit-1_d">[Density Limit 1]</a>: Since absolute overlapping is impossible — although Density Increase cannot be said to have a razor-sharp critical threshold — a limit naturally exists in Reality, and before reaching that limit, that compressed entity (Force, Pressure, Density) naturally disperses.

<a href="#Coupling-of-Pressure-Dispersion-and-Volume-Expansion_u" id="Coupling-of-Pressure-Dispersion-and-Volume-Expansion_d">[Coupling of Pressure Dispersion and Volume Expansion]</a>: Given equal quantity, lower Pressure or Density occupies a larger Volume than higher Pressure or Density. Therefore, Volume Expansion is naturally coupled with Pressure Dispersion (Density Dispersion).
...
* Additionally regarding 'gravity': First, substituting gravity into the above proposition yields 'matter (particle) existence is the premise of force (gravity, attraction)', which was refuted above.  
  * And similarly, if 'matter (particle) existence is the premise of force (gravity, attraction)', likewise, 'for gravity or attraction to exist, matter (particles) must exist first'. In this work, it was stated that "'*matter (particle) existence is not the premise of force'; rather, 'force precedes matter'*". If this is true, as long as conventional gravity (attraction) is preconditioned, the Formation of the Universe or matter (particles) cannot be discussed.  
  * 'Gravity', for instance, is a macroscopic compression Actuality resulting from Inward Pressure toward Earth being larger than Outward Pressure. Due to this Pressure imbalance, all internal elements gather to form a single Object (Earth), and *the phenomenon of compression (Actuality) due to this imbalance is what is called gravity*, which is not true.<a href="#Gravity_d" id="Gravity_u"><sup>[Gravity]</sup></a>
...
#### **1\. Accumulation and Great Expansion in the Nothingness State<a href="#Nothingness-State_d" id="Nothingness-State_u"><sup>[Nothingness State]</sup></a>**

<a href="#Nothingness-State_u" id="Nothingness-State_d">[Nothingness State]</a>: \[Definition\] Bound to Maintenance<a href="#Maintenance_d" id="Maintenance_u"><sup>[Maintenance]</sup></a>, referring to a State where Volume is `0 < Volume of Nothingness State < 1`. This cannot be reproduced.

<a href="#Maintenance_u" id="Maintenance_d">[Maintenance]</a>: \[Definition\] In this work, 'Maintenance' does not mean an active act. Bound to Reality<a href="#Pressure_d" id="Pressure_u"><sup>[Pressure]</sup></a>, it refers to a State continuing through an unerasable (`cannot be 0`) Minimum Interval (`𝛅_min`). And this Interval is called the Maintenance Interval (`𝛅`).

<a href="#Pressure_u" id="Pressure_d">[Pressure]</a>: \[Definition\] 'Pressure' in this work is not subordinate to conventional density concepts preconditioned on space (volume) or particles (mass density). It refers solely to 'singular or plural 1-Unit Force possessing Directionality'. For example, stronger Pressure means a State where more such 1-Unit Forces are overlapped and Accumulated.

* That the Universe is Non-fixed (No-holder, not absolutely at rest<a href="#Refutation-of-Absolute-Rest-2_d" id="Refutation-of-Absolute-Rest-2_u"><sup>[Refutation of Absolute Rest 2]</sup></a>) is perpetual. In other words, the Universe is the perpetuity of Transition (`→`)<a href="#Transition_d" id="Transition_u"><sup>[Transition]</sup></a>.

<a href="#Refutation-of-Absolute-Rest-2_u" id="Refutation-of-Absolute-Rest-2_d">[Refutation of Absolute Rest 2]</a>: In this work, 'absolute rest of Reality' is a contradictory proposition. Hypothetically extrapolating, if all Reality inside the Universe is interfused osmose-like, if one Reality came to absolute rest — since being at rest means having no Interval, Interval as much as the rested Reality portion vanishes — due to Pressure \[due to infinite (?) drop in Interval\], all Intervals would be sucked into that rested Reality portion \[the entire Universe sucked in\] ... and the Universe could not exist in reality.

<a href="#Transition_u" id="Transition_d">[Transition]</a>: \[Definition\] If there were no 'Transition', nothing would happen and the current Universe could not exist in reality. And 'Transition (`→`)' naturally possesses Directionality and is Pressure. And in the Nothingness State, this Pressure is Outward Pressure<a href="#Original-Pressure_d" id="Original-Pressure_u"><sup>[Original Pressure]</sup></a>. And 'Transition' possesses an Interval (`t₁ - t₀`), termed the Transition Interval<a href="#Transition-Interval_d" id="Transition-Interval_u"><sup>[Transition Interval]</sup></a>.

<a href="#Original-Pressure_u" id="Original-Pressure_d">[Original Pressure]</a>: \[Definition\] Refers to the Directionality caused by the Universe being Non-fixed, the misalignment (Phase Shift) caused by this Directionality, the Transition (`→`) caused by this misalignment, and the Pressure perpetually generated by this Transition.

<a href="#Transition-Interval_u" id="Transition-Interval_d">[Transition Interval]</a>: \[Definition\] Refers to the 'unerasable Interval (`cannot be 0`) bound to Reality' regarding Transition.<a href="#Unerasable-Interval_d" id="Unerasable-Interval_u"><sup>[Unerasable Interval]</sup></a>

<a href="#Unerasable-Interval_u" id="Unerasable-Interval_d">[Unerasable Interval]</a>: ➀ Maintenance (`t₀ → t₁`) is also a Transition (`→`), having an Interval (`t₁ - t₀`), and Absolute Nothingness lacking even this Interval (`t₁ - t₀`) \[Maintenance Interval (`𝛅`) or Minimum Interval (`𝛅_min`)\] is impossible. Therefore, ➁ Nothingness naturally entails Maintenance (Maintenance Interval). Therefore, ➂ even in the Nothingness State, Interval is not `0`, and Volume is also not `0`.

* `Interval ≡ Transition(→) ≡ Pressure(P) ⇔ Density(𝛒)`  
* That the Nothingness State is maintained (`t₀ → t₁`) (Phase Shift) (hereafter, that corresponding to `t₀ → t₁` is called 1-Unit) is also a Transition (`→`) (Pressure). In the Nothingness State (`0 < Volume < 1`), Original Pressure accumulates (imperfect Phase Shift or imperfect Phase accumulation). And Original Pressure reaches limit Density<a href="#Apexon_d" id="Apexon_u"><sup>[Apexon]</sup></a>. Even in such a State, naturally Original Pressure occurs, and Apexon, unable to accumulate further, undergoes 1-Unit Fusion instead. And by that impact, all Phases undergo 1-Unit Fission, causing the Great Expansion.

<a href="#Apexon_u" id="Apexon_d">[Apexon]</a>: \[Definition\] Refers to the maximum limit Density state accumulated under Nothingness Volume (`0 < Volume < 1`). Possible only in the Nothingness State because post-Formation Volume introduces two variables (Volume and Pressure) instead of one constant Volume (`0 < V < 1`).

#### **2\. Formation of the Universe**

* ➀ Due to the Great Expansion, total Volume becomes low-density, with its center even lower in Density. ➁ Upon Great Expansion (when Inward Surface Area expands), corresponding larger Original Pressure<a href="#Conversion-of-Original-Pressure-to-Inward-Pressure_d" id="Conversion-of-Original-Pressure-to-Inward-Pressure_u"><sup>[Conversion of Original Pressure to Inward Pressure]</sup></a> is generated. ➂ Due to increased Original Pressure, rapid contraction occurs, but Volume cannot return to Nothingness. ➃ Before internal Density fills up from increased Original Pressure, Boundary Dynamic Grid-lock occurs at the boundary<a href="#Boundary-Dynamic-Grid-lock-of-the-Universe_d" id="Boundary-Dynamic-Grid-lock-of-the-Universe_u"><sup>[Boundary Dynamic Grid-lock of the Universe]</sup></a>, forming the boundary, and thus the Universe is formed.<a href="#Quasi-fixed-State_d" id="Quasi-fixed-State_u"><sup>[Quasi-fixed State]</sup></a>

<a href="#Conversion-of-Original-Pressure-to-Inward-Pressure_u" id="Conversion-of-Original-Pressure-to-Inward-Pressure_d">[Conversion of Original Pressure to Inward Pressure]</a>: Post-Formation, Original Pressure acts as Inward Pressure because Phase Shift in any direction operates internally, as no external Volume (Space) exists outside the Universe.

<a href="#Boundary-Dynamic-Grid-lock-of-the-Universe_u" id="Boundary-Dynamic-Grid-lock-of-the-Universe_d">[Boundary Dynamic Grid-lock of the Universe]</a>: ➀ Original Pressure perpetuates. ➁ At the boundary of the Universe, it achieves Dynamic Equilibrium with total internal Retention Pressure, perpetually accumulating (increasing Density). ➂ When Density at the boundary reaches its limit, the Volume of the Universe expands.

<a href="#Quasi-fixed-State_u" id="Quasi-fixed-State_d">[Quasi-fixed State]</a>: \[Definition\] ➀ Pressure naturally possesses Directionality and interfuses. ➁ Directional Pressures collide into a Quasi-spherical Dynamic Grid-lock<a href="#Dynamic-Grid-lock_d" id="Dynamic-Grid-lock_u"><sup>[Dynamic Grid-lock]</sup></a>. ➂ This Dynamic Grid-lock acts as a Density Drop between interior and exterior. ➃ This drop acts as constraint strength against outward flow, driving inward convergent Helical Swirls toward the center. ➄ When Inward and Outward Pressures achieve Dynamic Equilibrium<a href="#Dynamic-Equilibrium_d" id="Dynamic-Equilibrium_u"><sup>[Dynamic Equilibrium]</sup></a> maintaining a Quasi-static state, this defines the Quasi-fixed State.

<a href="#Dynamic-Grid-lock_u" id="Dynamic-Grid-lock_d">[Dynamic Grid-lock]</a>: \[Definition\] The natural boundary or Density Drop formed by arch/inverse-arch (`^v^v`) configurations separating interior and exterior.

<a href="#Dynamic-Equilibrium_u" id="Dynamic-Equilibrium_d">[Dynamic Equilibrium]</a>: \[Definition\] The maintained interfusion (`^v^v`) where Inward Pressure cannot increase internal Density further, nor can the interior be dispersed.

* Since Inward Surface Area naturally exceeds Outward Area, Original Pressure perpetually infiltrates the interior as Retention Pressure<a href="#Asynchronous-Dual-Action_d" id="Asynchronous-Dual-Action_u"><sup>[Asynchronous Dual Action]</sup></a>. Newly infiltrated Retention Pressure forms new Density and Volume<a href="#Initial-Volume-Formation-and-Subsequent-Volume-Formation_d" id="Initial-Volume-Formation-and-Subsequent-Volume-Formation_u"><sup>[Initial Volume Formation and Subsequent Volume Formation]</sup></a>, accelerating Original Pressure accumulation.

<a href="#Asynchronous-Dual-Action_u" id="Asynchronous-Dual-Action_d">[Asynchronous Dual Action]</a>: Original Pressure exerts ➀ Immediate Boundary Action on the Inward Surface Area, while ➁ partially infiltrating perpetually as new Retention Pressure (Inflow Pressure)<a href="#Inflow-Pressure_d" id="Inflow-Pressure_u"><sup>[Inflow Pressure]</sup></a>, accumulating Delayed Retention Action affecting Density, Objects<a href="#Object_d" id="Object_u"><sup>[Object]</sup></a>, and Volume.

<a href="#Inflow-Pressure_u" id="Inflow-Pressure_d">[Inflow Pressure]</a>: \[Definition\] Pressure flowing into a Dynamic Grid-lock boundary. Pressure escaping outward is *Outflow Pressure*.

<a href="#Object_u" id="Object_d">[Object]</a>: \[Definition\] Entities with Density > 1 Unit formed into Quasi-spherical shapes.

<a href="#Initial-Volume-Formation-and-Subsequent-Volume-Formation_u" id="Initial-Volume-Formation-and-Subsequent-Volume-Formation_d">[Initial Volume Formation and Subsequent Volume Formation]</a>: Initial Volume Formation \= Imperfect Phase \+ Inward Pressure. Subsequent Volume Formation \= Retention Pressure \+ Inward Pressure.
...
* Inside the Universe, directionalities of Boundary Dynamic Grid-lock and Retention Pressure form small Objects. Flow toward dominant pressures creates Direct Flow, generating small and larger Objects, which collision-trigger Macro Vortices (Vortex) and Micro Eddies (Eddy). And perpetually: Object formation and Direct Flow by Boundary Dynamic Grid-lock; Object and Vortex formation by Direct Flow; Object and Eddy formation by Vortex; Object and smaller Eddy formation by Eddy; 1-Unit Fusion (interfusion) by Boundary Dynamic Grid-lock; Vortex-Vortex or Vortex-Eddy fusion by Direct Flow; Eddy fusion by Vortex; smaller Eddy fusion by Eddy.  
  ...
  * Bottom-up coupling: `Micro Eddy + Micro Eddy ⇔ Eddy + Eddy or Micro Eddy ⇔ Vortex + Vortex or Eddy or Micro Eddy ⇔ Direct Flow ⇔ Boundary Dynamic Grid-lock`  
  * `'[Nothingness State + Maintenance₁ {Transition₁(→₁), Original Pressure₁}]<a href="#Formon_d" id="Formon_u"><sup>[Formon]</sup></a> ⇒ [Pressure Increase ⇔ Density Increase] ...' = 'Nothingness State + Maintenance₁ ⇒ Nothingness State₁ + Maintenance₂ ⇒ Nothingness State₂ + Maintenance₃ ...' ⇒ Apexon Reached ⇒ Great Expansion [Density Dispersion ≡ Volume Expansion] ⇒ Pressure Increase ⇔ Density Increase<a href="#Density-Limit-2_d" id="Density-Limit-2_u"><sup>[Density Limit 2]</sup></a> ≡ Volume Contraction ⇒ Retention Pressure [Outward Pressure] overcomes Inward Pressure ⇒ Density Dispersion ≡ Volume Expansion ⇒ Pressure Increase ⇔ Density Increase ≡ Volume Contraction ⇒ Retention Pressure [Outward Pressure] overcomes Inward Pressure ⇒ Density Dispersion ≡ Volume Expansion ⇒ Pressure Increase ⇒ ...` Fractal

<a href="#Formon_u" id="Formon_d">[Formon](𝔽)</a>: \[Definition\] May be understood as replacing action-points or actions at action-points, but Formon does not mean static coordinates or segmented nodes. It refers to the occurrence of Pressure itself, collisions of Pressure, or transitions between antecedent and subsequent — *the manifestation and event itself*. For example, manifestation of Original Pressure, collision of Pressure A with something or Pressure B, or transition from antecedent to subsequent due to collision — refers to the manifestation/collision itself. Furthermore, since the Universe is perpetually `Nothingness State + Maintenance [Transition₁(→₁), Original Pressure₁]`, *the Universe itself is a single macro Formon (`𝔽`), representing Reality perpetuating beyond temporal segmentation.<a href="#Perpetuity-of-Formon_d" id="Perpetuity-of-Formon_u"><sup>[Perpetuity of Formon]</sup></a>*

<a href="#Perpetuity-of-Formon_u" id="Perpetuity-of-Formon_d">[Perpetuity of Formon]</a>: The Universe is Pressure \[mass (?) of Pressure\], with no Space outside. Present is also `Nothingness State (Space₀) + Maintenance (Σ Retention Pressure + Original Pressure)` (Formon). Thus, `Nothingness State + Maintenance = Formon`, Perpetuity of Formon.

<a href="#Density-Limit-2_u" id="Density-Limit-2_d">[Density Limit 2]</a>: Density limit ranges minimally from 1-Unit partial overlap to maximally less than Apexon: `1 Unit < Density < Apexon`.

* `[Retention Pressure + (Inflow Pressure - Outflow Pressure)] / ρ (where 1 Units < ρ < Apexon) ⟺ ky`
